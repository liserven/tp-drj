...
class Memcache extends MemcachePool  {
	...
	/**
	 * Open memcached service connection
	...
	/**
	 * Open memcached service persistent connection
	...
	/**
	 * Add a memcached service to connection pool
	 ...
	 * Number of buckets to create for this service which in turn control its
	 ...
	 * Controls how often a failed service will be retried, the default value
	 ...
	 * effect is that each web service child will retry the connection about every
	 * retry_interval seconds when serving a page.
	 * </p>
	 * @param status bool[optional] <p>
	 * Controls if the service should be flagged as online. Setting this parameter
	 * to false and retry_interval to -1 allows a failed 
	 * service to be kept in the pool so as not to affect the key distribution
	 * algorithm. Requests for this service will then failover or fail immediately
	 * depending on the memcache.allow_failover setting.
	 * Default to true, meaning the service should be considered online.
	 * </p>
	 * @param failure_callback callable[optional] <p>
	 * Allows the user to specify a callback function to run upon encountering an 
	 * error. The callback is run before failover is attempted. The function takes 
	 * two parameters, the hostname and port of the failed service.
	 * </p>
