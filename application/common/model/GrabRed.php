...
class GrabRed extends BaseModel
{
    ...
    {
        return self::alias('gb')
            ->join('__GIVE_RED__ gr', 'gr.id=gb.rid', 'right')
            ->join('__USER_DATA__ ud', 'ud.ud_id=gr.user_id', 'left')
            ->where($where)
            ->field('ud.ud_name, ud.ud_logo, ud.ud_phone, ud.ud_sex, ud.ud_id, gr.solo, gr.word, gr.num,ud.county,gb.create_at,gr.user_id, gr.id, ud.city, ud.town
                    ')
           ->find();
    }
}